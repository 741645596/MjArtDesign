...
public class BigPicCheckEditorWindow : AssetCheckEditorWindowBase<BigPicAssetInfo>
{
    ...
    private void _ShowRuleDes()
    {
        const string s_Des = "检查规则：\n" +
            ...
            "2、纹理无透明度(比如背景图、3D模型)，则Alpha Source建议设置为None，可以节省内存。如果导入的图片带有alpha通道还会减少包体大小\n" +
            ...
            "5、纹理通常不需要开启读写(R&W)选项，这个会导致纹理内存翻倍。规定：图片名称后缀以_rw结尾表示需要开启RW，这样一键修复就不会关闭RW\n" +
            "6、Mask贴图和噪声图规范使用单通道，且使用Red通道，文件名包含'_sc_'用来识别单通道\n" +
