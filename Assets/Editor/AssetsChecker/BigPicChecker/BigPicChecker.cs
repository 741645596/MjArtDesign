...
public static class BigPicChecker
{
    ...
    public static BigPicAssetInfo GetAssetInfo(string file)
    {
        ...
        info.anisoLevel = importer.anisoLevel;
        ...
        info.webGLFormat = webgl.format;
        info.isWebGLOverride = webgl.overridden;
        ...
        info.isNeedSingleChannel = _IsNeedSingleChannel(importer, file);
    ...
    public static void SetTextureFormat(TextureImporter importer, TextureImporterFormat newFormat)
    {
        ...
        webglSettings.format = newFormat;
    ...
    public static void SetOverridden(TextureImporter importer, bool isOverriden)
    {
        ...
        // WebGL设置
        var webglSettings = importer.GetPlatformTextureSettings("WebGL");
        webglSettings.overridden = isOverriden;
        importer.SetPlatformTextureSettings(webglSettings);
    }
    ...
    // 规定：文件名称包含_sc_，表示需要开启单通道，且使用Red通道
    private static bool _IsNeedSingleChannel(TextureImporter importer, string file)
    {
        if (file.Contains("_sc_") == false)
        {
            return false;
        }
