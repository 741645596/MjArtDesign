...
public class AudioAssetInfo : AssetInfoBase
{
    ...
    public const float WebGL_Quality_Recommon = 0.25f;
    ...
    // WebGL需要特殊处理
    public AudioImporterSampleSettings sampleSettingsWebGL;
    public bool isOverrideWebGL;
    ...
    public override void Fix()
    {
        ...
        // WebGL
        if (isOverrideWebGL == false ||
            sampleSettingsWebGL.quality > WebGL_Quality_Recommon)
        {
            var format = audioLength >= Time_Threshold ?
                AudioClipLoadType.Streaming :
                AudioClipLoadType.DecompressOnLoad;
            sampleSettingsWebGL.loadType = format;
            sampleSettingsWebGL.quality = WebGL_Quality_Recommon;
            isOverrideWebGL = importer.SetOverrideSampleSettings("WebGL", sampleSettingsWebGL);
        }
    ...
    public override string GetErrorDes()
    {
        ...
        if (isOverrideWebGL == false ||
            sampleSettingsWebGL.quality > WebGL_Quality_Recommon)
        {
            errors.Add($"WebGL平台需Override且Quality={WebGL_Quality_Recommon*100}");
        }
    ...
    public override bool IsError()
    {
        ...
        // WebGL
        Debug.Log($"==== {isOverrideWebGL}, {sampleSettingsWebGL.quality}");
        if (isOverrideWebGL == false ||
            sampleSettingsWebGL.quality > WebGL_Quality_Recommon)
        {
            return true;
