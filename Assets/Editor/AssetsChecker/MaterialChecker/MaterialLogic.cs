...
public static class MaterialLogic
{
    ...
    public static List<string> GetRedundanceKeywords(Material material)
    {
        ...
        // unity 2021材质球冗余属性改为m_InvalidKeywords
        SerializedObject psSource = new SerializedObject(material);
        SerializedProperty invalidKeywordsProp = psSource.FindProperty("m_InvalidKeywords");
        if (invalidKeywordsProp != null)
        {
            foreach (var p in invalidKeywordsProp)
            {
                var val = p as SerializedProperty;
                notExistKeywords.Add(val.stringValue);
            }
    ...
    public static void FixRedundanceKeywords(string matPath)
    {
        ...
        if (invalidKeywordsProp != null)
        {
            invalidKeywordsProp.ClearArray();
    ...
    /// <param name="material"></param>
    /// <returns></returns>
    private static List<string> GetNeedKeywords(Material material)
    {
