...
public class AtlasAssetInfo : AssetInfoBase
{
    ...
    // 允许单张碎图最大尺寸
    public const int Max_Single_Size = 400 * 400; 
    ...
    // 合图文件尺寸集合
    public List<Vector2> fileSizes;

    // 是否单张碎图过大
    public bool isOverMaxSingleSize
    {
        get
        {
            foreach (var s in fileSizes)
            {
                if (s.x * s.y > Max_Single_Size)
                {
                    return true;
                }
            }
    ...
    public override string GetErrorDes()
    {
        ...
        if (isOverMaxSingleSize)
        {
            desArr.Add($"单图尺寸超过400x400");
        }
    ...
    public override bool IsError()
    {
        ...
        if (isOverMaxSingleSize) return true;
    ...
    private static bool _SetSprite2DAndCloseOverride(string assetPath)
    {
        ...
        webGLSettings.overridden = false;
