...
public static class AtlasChecker
{
    ...
    public static AtlasAssetInfo GetAssetInfo(string file)
    {
        ...
        if (string.IsNullOrEmpty(dir))
        {
            ...
            info.fileSizes = new List<Vector2>();
        }
        else
        {
            ...
            info.fileSizes = GetFileSizes(targetFiles);
    ...
    /// <param name="filePaths"></param>
    /// <returns></returns>
    public static List<Vector2> GetFileSizes(List<string> filePaths)
    {
        var vecArr = new List<Vector2>();
        foreach (var path in filePaths)
        {
            var a = BigPicChecker.GetTextureSize(path);
            vecArr.Add(a);
    ...
    public static string GetSpriteAtlasPackDir(string atlasPath)
    {
        ...
        if (packs.Length != 1)
        {
            Debug.LogWarning($"合图文件{atlasPath}，配置了{packs.Length}个路径。请按规范：将合图文件放在合图文件夹内，且只配置合图文件夹",
                AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(atlasPath));
            return null;
        }
