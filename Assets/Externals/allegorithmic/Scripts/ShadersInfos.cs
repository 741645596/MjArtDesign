		...
		{
			{
				...
				new ShaderInfos()
				{
					...
					ExportPreset = "Unity 5 (Standard Metallic)_TextureAssign",

                    PropertiesAssociation = new Dictionary<string, string>() {
						// Associate texture name
						{ "$textureSet_AlbedoTransparency", "_MainTex" },
						{ "$textureSet_Emission", "_EmissionMap" },
						{ "$textureSet_MetallicSmoothness", "_MetallicGlossMap" },
						{ "$textureSet_Normal", "_BumpMap" },
                        { "$textureSet_AO","_OcclusionMap"}
					},
					PostProcesses = new Dictionary<string, ShaderInfos.PropertyChangedPostProcess>() {
                        ...
                        { "_OcclusionMap", (m, d) => m.EnableKeyword("_OCCLUSIONMAP")}
					},
