...
public static class CameraExtension
{
    ...
    public static Matrix4x4 GetProjectionMatrix(this Camera camera, float renderScale, float jitterX, float jitterY)
    {
        if (camera == null)
            return Matrix4x4.identity;

        var originProj = camera.projectionMatrix;
        var planes = originProj.decomposeProjection;
        float vertFov = Math.Abs(planes.top) + Math.Abs(planes.bottom);
        float horizFov = Math.Abs(planes.left) + Math.Abs(planes.right);
        float width = camera.scaledPixelWidth * renderScale;
        float height = camera.scaledPixelHeight * renderScale;
        var planeJitter = new  Vector2(jitterX * horizFov / width, jitterY * vertFov / height);
