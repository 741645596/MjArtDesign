		...
		SubShader
		{
			...
			/*Stencil
			{
				...
				half4 frag(VertexOutput IN, float facing : VFACE) : SV_Target
				{
					...
					float diffuseTerm = NoL * 0.5f + 0.5f;
					float3 diffuse = diffuseTerm * _u_DirectLightIntensity;
					...
					// F项
					float  NF = saturate(Pow5(1.0f - abs(NoV)));
			...
			Pass
			{
				...
				v2f vert(a2v input)
				{
					...
					output.uv = TRANSFORM_TEX(input.texcoord, _u_Albedo);
				...
				{
					float  Alpha  = tex2D(_u_Albedo, i.uv).a * _BaseColor.a;
				    clip(Alpha - _Cutoff);
					return 0;
				}
				ENDHLSL
