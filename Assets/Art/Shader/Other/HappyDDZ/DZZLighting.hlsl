#ifndef UNIVERSAL_DZZ_LIGHTING_INCLUDED
#define UNIVERSAL_DZZ_LIGHTING_INCLUDED
...
half3 GlossyEnvironmentReflection(half3 reflectVector, half perceptualRoughness, half occlusion, samplerCUBE _GlobalCubemap, float4 _GlobalCubemap_HDR)
{
#if !defined(_ENVIRONMENTREFLECTIONS_OFF)
    half mip = PerceptualRoughnessToMipmapLevel(perceptualRoughness);
    half4 encodedIrradiance = texCUBElod(_GlobalCubemap, half4(reflectVector, mip));

#if defined(UNITY_USE_NATIVE_HDR)
    half3 irradiance = encodedIrradiance.rgb;
#else
    half3 irradiance = DecodeHDREnvironment(encodedIrradiance, _GlobalCubemap_HDR);
#endif
...
half3 GlobalIllumination(BRDFData brdfData, 
    ...
    samplerCUBE _GlobalCubemap, float4 _GlobalCubemap_HDR,
    half _GlobalCubemapRotation, half _GlobalCubemapIntensity)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    // 对反射方向进行了旋转。
    half angle = _GlobalCubemapRotation * 6.28f;
    half conAngle = cos(angle);
    half sinAngle = sin(angle);

    half x= dot(half2(conAngle, -sinAngle), reflectVector.xy);
    half z= dot(half2(sinAngle, conAngle), reflectVector.xy);
    ...
    half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, brdfData.perceptualRoughness, occlusion, _GlobalCubemap, _GlobalCubemap_HDR) * _GlobalCubemapIntensity;
...
half3 LightingPhysicallyBased(BRDFData brdfData, 
    ...
    half _GlobalBackfaceBrightness)
{
    half NdotL = saturate(dot(normalWS, lightDirectionWS));
    // 改动的地方了。
    half3 radiance = lightColor * lightAttenuation *(NdotL + (1- NdotL) * _GlobalBackfaceBrightness);
...
half3 LightingPhysicallyBased(BRDFData brdfData,
    ...
    half _GlobalBackfaceBrightness)
{
    return LightingPhysicallyBased(brdfData, light.color, light.direction, light.distanceAttenuation, normalWS, viewDirectionWS, specularHighlightsOff, _GlobalBackfaceBrightness);
}
...
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData,
    samplerCUBE _GlobalCubemap, float4 _GlobalCubemap_HDR,
    half _GlobalBackfaceBrightness,half _GlobalCubemapRotation, half _GlobalCubemapIntensity,
    half3 _AmbientCol)
{
    ...
    half3 color = GlobalIllumination(brdfData, 
                                     ...
                                     _GlobalCubemap, _GlobalCubemap_HDR,
                                     _GlobalCubemapRotation, _GlobalCubemapIntensity);
    color += LightingPhysicallyBased(brdfData,
                                     mainLight,
                                     inputData.normalWS, inputData.viewDirectionWS,
                                     specularHighlightsOff,
                                     _GlobalBackfaceBrightness);
    ...
    half3 _AmbientCol)
{
    ...
    return UniversalFragmentPBR(inputData, s, _GlobalCubemap, _GlobalCubemap_HDR, _GlobalBackfaceBrightness, _GlobalCubemapRotation, _GlobalCubemapIntensity, _AmbientCol);
}
