...
Shader "URP/Realtime/Scene/Unlit_ReceiveShadow"
{
  Properties
  {
    [Toggle(ALPHATEST_ON)] _AlphaTest ("AlphaTest", float) = 1
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat0_d.xyz = ((_Color.xyz * u_xlat16_0.xyz) + (-_ShadowColor.xyz));
          u_xlat3 = ((-_ShadowColor.w) + 2);
          #ifdef UNITY_ADRENO_ES3
          u_xlat3 = min(max(u_xlat3, 0), 1);
          #else
          u_xlat3 = clamp(u_xlat3, 0, 1);
          #endif
          u_xlat0_d.xyz = ((float3(u_xlat3, u_xlat3, u_xlat3) * u_xlat0_d.xyz) + _ShadowColor.xyz);
