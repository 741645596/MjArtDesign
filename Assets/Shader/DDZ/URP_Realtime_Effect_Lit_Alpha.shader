...
Shader "URP/Realtime/Effect/Lit_Alpha"
{
  Properties
  {
    [MaterialEnum(Front, 2, Off, 0)] _CullMode ("Render Mode", float) = 2
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          #endif
          u_xlat16_2.xyz = ((u_xlat16_0.www * u_xlat16_7.xyz) + u_xlat16_3_d.xyz);
          out_f.color.xyz = (u_xlat6.www * u_xlat16_2.xyz);
          #ifdef UNITY_ADRENO_ES3
          out_f.color.xyz = min(max(out_f.color.xyz, 0), 1);
          #else
          out_f.color.xyz = clamp(out_f.color.xyz, 0, 1);
          #endif
          out_f.color.w = (u_xlat6.w * u_xlat6.w);
          #ifdef UNITY_ADRENO_ES3
